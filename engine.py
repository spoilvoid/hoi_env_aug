...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, max_norm: float = 0, branch_criterion:torch.nn.Module = None):
    # 转换为训练模式
    model.train()
    criterion.train()
    if branch_criterion != None:
        branch_criterion.train()
    # 记录日志与设置
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # 如果没有采用环境感知部分,那么adaptive_alpha为None
        outputs, adaptive_alpha = model(samples)
        loss_dict = criterion(outputs, targets)
        weight_dict = criterion.weight_dict
        losses = sum(loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if k in weight_dict)
        if branch_criterion != None:
            branch_loss = branch_criterion(adaptive_alpha, targets)
            losses = losses + branch_loss
